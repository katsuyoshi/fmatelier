...
Sending a 32-voice bulk dump (4104 bytes) to a real DX7 may result in **data corruption in some voices**. This is caused by the USB-MIDI interface sending SysEx data in bursts too fast for the DX7 to process.

The Web MIDI API does not provide byte-level flow control, so the data rate cannot be throttled from the browser.

**Workarounds:**

- Use the Arduino MIDI interface included in the [`arduino/`](arduino/) directory. Place an Arduino (Uno R4 + MIDI Shield) between your PC and DX7. This allows 32-voice bulk dumps to be sent without data corruption
- Use **Send Voice** to transfer individual voices to the DX7's edit buffer, then store them manually

**Note:** The .syx files generated by FMAtelier are 100% correct. This was verified by byte-for-byte comparison with original DX7 ROM cartridge dumps (0 differences). The issue is caused by the MIDI interface transmission speed, not the file data.
